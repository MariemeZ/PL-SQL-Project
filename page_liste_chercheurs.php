...
$queryChercheurs = "SELECT * FROM CHERCHEUR1";
...
while ($row = oci_fetch_assoc($resultChercheurs)) {
    ...
    echo '<td>
    <button><a href="page_modification_chercheur.php?chno=' . $row['CHNO'] . '">Modifier</a></button>
    <button onclick="supprimerChercheur(' . $row['CHNO'] . ')">Supprimer</button>
</td>';
...
function supprimerChercheur(chno) {
    ...
    if (confirm("Êtes-vous sûr de vouloir supprimer ce chercheur?")) {
        ...
        fetch('supprimer_chercheur1.php', {
            method: 'POST',
            headers: {
