      ...
      <div class="container">
        <div class="row no-gutters slider-text js-fullheight align-items-end justify-content-start">
          <div class="col-md-9 ftco-animate pb-5">
            ...
            <h1 class="mb-3 bread">Liste des publications</h1>
          </div>
        </div>
      </div>
    </section>
...
// Récupérer les données de la table "publication"
$queryPublications = "SELECT * FROM PUBLICATION1";
$resultPublications = oci_parse($conn, $queryPublications);
oci_execute($resultPublications);
?>
<table border="1" style="margin: 26px auto;">
    <tr>
        <th>Pubno</th>
        <th>Titre</th>
        <th>Theme</th>
    ...
    <?php
    while ($row = oci_fetch_assoc($resultPublications)) {
        echo '<tr>';
        echo '<td>' . $row['PUBNO'] . '</td>';
        echo '<td>' . $row['TITRE'] . '</td>';
        echo '<td>' . $row['THEME'] . '</td>';
        echo '<td>' . $row['TYPE_PUB'] . '</td>';
        echo '<td>' . $row['VOLUME'] . '</td>';
        echo '<td>' . $row['DATE_PUB'] . '</td>';
        echo '<td>' . $row['APPARITION'] . '</td>';
        echo '<td>' . $row['EDITEUR'] . '</td>';
        echo '<td>
            <button><a href="page_modification_publication.php?pubno=' . $row['PUBNO'] . '">Modifier</a></button>
            <button onclick="supprimerPublication(\'' . $row['PUBNO'] . '\')">Supprimer</button>
        </td>';
        echo '</tr>';
    }
    ?>
...
<script>
    function supprimerPublication(id) {
        ...
        if (confirm("Êtes-vous sûr de vouloir supprimer cette publication?")) {
            ...
            fetch('supprimer_publication.php', {
                method: 'POST',
                headers: {
